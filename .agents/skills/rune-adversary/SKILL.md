...
If any auth, crypto, or payment logic is in the plan: MUST call `the rune-sentinel skill` for deep analysis.
...
If bottleneck patterns detected: call `the rune-perf skill` for quantitative analysis.
...
- Use `the rune-scout skill` to find all files the plan will modify or depend on
