...
Apply code changes. Fix receives a plan, debug finding, or review finding and writes the actual code. It does NOT investigate root causes — that is the rune-debug skill's job. Fix is the action hub: locate, change, verify, report.
...
If unsure whether the test is wrong or the implementation is wrong → call `the rune-debug skill` to investigate.
</HARD-GATE>
...
- If the request is ambiguous or root cause is unclear → call `the rune-debug skill` before proceeding
...
| `LOGIC_ERROR` — wrong output, incorrect calculation, bad algorithm | **INVESTIGATE** | Do NOT auto-fix. Call `the rune-debug skill` — logic errors need root cause analysis. |
...
- Use `the rune-scout skill` to locate the relevant files, functions, and surrounding code
...
- If tests fail after the fix:
  - Investigate with `the rune-debug skill` (max 3 debug loops before escalating)
...
If `the rune-debug skill` left `#region agent-debug` markers in the code:
...
- Call `the rune-hallucination-guard skill` to verify all imports introduced or modified are real and correctly named
- Call `the rune-docs-seeker skill` if any external API, library method, or SDK call was added or changed
- For complex or risky fixes (auth, data mutation, async logic): call `the rune-review skill` for a full quality check
...
4. MUST NOT exceed 3 fix attempts — if 3 fixes fail, re-diagnose via the rune-debug skill (which will classify: wrong approach → brainstorm rescue, wrong design → plan redesign)
...
|------|----------|------------|
| Evidence Gate | Debug report OR clear error description before fixing | Run the rune-debug skill first |
