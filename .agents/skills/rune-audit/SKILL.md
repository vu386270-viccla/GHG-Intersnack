...
- `retro` (L2): Phase 6 — engineering velocity and health dimension (the rune-retro skill)
...
Call `the rune-scout skill` for a full project map. Then use read the file on:
...
Call `the rune-journal skill` to record: audit date, overall health score, verdict, and CRITICAL count.
...
| Security Gate | sentinel report received before assembling final report | Invoke the rune-sentinel skill — do not skip |
| Deps Gate | dependency-doctor report received before assembling final report | Invoke the rune-dependency-doctor skill — do not skip |
...
| Journal entry | Text | `.rune/adr/` (via `the rune-journal skill`) |
