...
Code quality analysis. Review finds bugs, bad patterns, security issues, and untested code. It does NOT fix anything — it reports findings and delegates: bugs go to the rune-fix skill, untested code goes to the rune-test skill, security-critical code goes to the rune-sentinel skill.
...
- If context is unclear: use `the rune-scout skill` to identify all files touched by the change
...
| 50+ callers | Critical | MUST escalate to adversarial analysis (the rune-adversary skill) even in quick triage |
...
- If any security-sensitive code found (auth logic, input handling, crypto, payment): call `the rune-sentinel skill` for deep scan
...
- If untested code found: call `the rune-test skill` with specific instructions on what to test
...
- Apply AUTO-FIX findings directly via `the rune-fix skill` — include all in a single batch
...
After reporting:
- If any CRITICAL findings: call `the rune-fix skill` immediately with the finding details
- If any HIGH findings: call `the rune-fix skill` with the finding details
- If untested code: call `the rune-test skill` with specific coverage gaps identified
...
7. MUST flag untested code paths and recommend tests via the rune-test skill
...
| Security finding without sentinel escalation | HIGH | Any auth/crypto/payment code touched → MUST call the rune-sentinel skill |
