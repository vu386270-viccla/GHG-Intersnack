...
Call `the rune-scout skill` targeting the module to refactor. Ask scout to return:
...
Confirm that `the rune-safeguard skill` has already run for this module (check for `tests/char/<module>.test.ts` and `rune-safeguard-<module>` git tag).

If safeguard has NOT run, call `the rune-safeguard skill` now before continuing. Do not skip this.
...
- If a change reveals a hidden bug, stop and call `the rune-debug skill` before continuing
...
After every edit the file, call `the rune-test skill` targeting:
...
Call the rune-debug skill to investigate. Fix before next edit.
The code MUST stay in a working state after every single change.
```

### Step 5 — Review

After all edits for this session are complete and tests pass, call `the rune-review skill` on the changed files.
...
Call `the rune-journal skill` to record:
- Module operated on
- Pattern used
- Files changed
...
- Recommended: re-run the rune-surgeon skill targeting [module] — session 2

### Next Step
[if complete]: Run the rune-autopsy skill to update health scores
