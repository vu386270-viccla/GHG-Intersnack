...
1. **Scan installed skills**: find files by pattern for `skills/*/skill.md` (core L0-L3) and `extensions/*/PACK.md` (L4 packs)
2. **Scan paid extensions**: find files by pattern for `extensions/pro-*/PACK.md` (Pro/Business packs — only present if purchased)
...
|---|---|---|
| Build / implement / add feature / fix bug | `the rune-cook skill` | Any code change request |
| Large multi-part task / parallel work | `the rune-team skill` | 5+ files or 3+ modules |
| Deploy + launch + marketing | `the rune-launch skill` | Ship to production |
| Legacy code / rescue / modernize | `the rune-rescue skill` | Old/messy codebase |
| Check project health / full audit | `the rune-audit skill` | Quality assessment |
| New project / bootstrap / scaffold | `the rune-scaffold skill` | Greenfield project creation |

**Default route**: If unclear, route to `the rune-cook skill`. Cook handles 70% of all requests.
...
|---|---|---|
| Plan / design / architect | `the rune-plan skill` | L2 — requires opus |
| Brainstorm / explore ideas | `the rune-brainstorm skill` | L2 — before plan |
| Review code / check quality | `the rune-review skill` | L2 |
| Write tests | `the rune-test skill` | L2 — TDD |
| Refactor | `the rune-surgeon skill` | L2 — incremental |
| Deploy (without marketing) | `the rune-deploy skill` | L2 |
| Security concern | `the rune-sentinel skill` | L2 — opus for critical |
| Performance issue | `the rune-perf skill` | L2 |
| Database change | `the rune-db skill` | L2 |
| Received code review / PR feedback | `the rune-review-intake skill` | L2 |
| Protect / audit / document business logic | `the rune-logic-guardian skill` | L2 |
| Create / edit a Rune skill | `the rune-skill-forge skill` | L2 — requires opus |
| Incident / outage | `the rune-incident skill` | L2 |
| UI/UX design | `the rune-design skill` | L2 |
| Fix bug / debug only (no fix) | `the rune-debug skill` → `the rune-fix skill` | L2 chain |
| Marketing assets only | `the rune-marketing skill` | L2 |
| Gather requirements / BA / elicit needs | `the rune-ba skill` | L2 — requires opus |
| Generate / update docs | `the rune-docs skill` | L2 |
| Build MCP server | `the rune-mcp-builder skill` | L2 |
| Red-team / challenge a plan / stress-test | `the rune-adversary skill` | L2 — requires opus |
...
|---|---|---|
| `the rune-scout skill` | cook, plan, team | Codebase scanning |
| `the rune-fix skill` | debug, cook | Apply code changes |
| `the rune-preflight skill` | cook | Quality gate |
| `the rune-verification skill` | cook, fix | Run lint/test/build |
| `the rune-hallucination-guard skill` | cook, fix | Verify imports |
| `the rune-completion-gate skill` | cook | Validate claims |
| `the rune-sentinel-env skill` | cook, scaffold, onboard | Environment pre-flight |
| `the rune-research skill` / `the rune-docs-seeker skill` | any | Look up docs |
| `the rune-session-bridge skill` | cook, team | Save context (in-session state handoff) |
| `the rune-journal skill` | cook, team | Persistent work log within a session |
| `the rune-neural-memory skill` | cook, team, any L1/L2 | Cross-session cognitive persistence via Neural Memory MCP — semantic complement to session-bridge and journal |
| `the rune-git skill` | cook, scaffold, team, launch | Semantic commits, PRs, branches |
| `the rune-doc-processor skill` | docs, marketing | PDF/DOCX/XLSX/PPTX generation |
| "Done" / "ship it" / "xong" | — | `the rune-verification skill` → commit |
| "recall", "remember", "brain", "nmem", "cross-project memory" | `the rune-neural-memory skill` | Retrieve or persist cross-session context |
...
|---|---|---|
| `*.test.*`, `*.spec.*`, `__tests__/` | `the rune-test skill` | Test patterns, assertions, no `test.skip`, coverage rules |
| `migrations/`, `schema.*`, `*.prisma` | `the rune-db skill` | Migration safety, rollback script, parameterized queries |
| `Dockerfile`, `*.yml` (CI/CD), `terraform/` | `the rune-deploy skill` | Deployment checklist, no hardcoded secrets |
| `docs/*.md`, `README.md`, `CHANGELOG.md` | `the rune-docs skill` | Documentation patterns, no stale references |
| `SKILL.md`, `PACK.md` | `the rune-skill-forge skill` | Skill template compliance, frontmatter validation |
| `.env*`, `*secret*`, `*credential*` | `the rune-sentinel skill` | Security scan mandatory, never commit secrets |
...
Example: "Add auth and deploy it"
  → the rune-cook skill (add auth) FIRST
  → the rune-deploy skill SECOND (after cook completes)

Example: "Fix the login bug and add tests"
  → the rune-debug skill (diagnose) FIRST
  → the rune-fix skill (apply fix) SECOND
  → the rune-test skill (add tests) THIRD
...
1. Trigger `the rune-neural-memory skill` in **Capture Mode** automatically
...
**Auto-suggest rules:**
1. Only suggest if the pack's PACK.md **exists on disk** — find files by pattern for the pack path first. If not installed, skip silently.
