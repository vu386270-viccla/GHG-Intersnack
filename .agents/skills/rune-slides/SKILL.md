...
```bash
node .agents/skills/rune-slides-scripts/build-deck.js --input slides.json --output deck.md
```
...
**Fallback**: If `.agents/skills/rune-slides-scripts/build-deck.js` is unavailable, generate the Marp markdown directly — the script is an optimization, not a hard dependency.
...
2. DO NOT embed build-deck.js source in this skill — call it via `.agents/skills/rune-slides-scripts`
