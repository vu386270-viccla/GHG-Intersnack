...
Use read the file on `manifest.json`. For each declared permission, verify it is actually used in source code with search file contents. Flag any permission declared but not found in `*.ts` / `*.js` source files. Severity: HIGH.
...
Search file contents to find patterns that trigger automatic CWS rejection:
...
Find files by pattern to verify file existence. Run a shell command to check image dimensions with `file` or `identify` if ImageMagick is available.
...
Use read the file on existing source and `manifest.json` to determine:
...
Search file contents to find existing `chrome.runtime.sendMessage`, `chrome.tabs.sendMessage`, and `chrome.runtime.connect` calls. Map the full message topology:
...
Use read the file on any existing `manifest.json` or project description to classify the extension type:
