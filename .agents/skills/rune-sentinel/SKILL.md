...
If `.rune/` directory exists, invoke `the rune-integrity-check skill` (L3) on all `.rune/*.md` files and any state files in the commit diff.
...
1. read the file `.rune/contract.md` and parse each `## section` as a named rule set
...
1. read the file `.rune/org/org.md` and extract the `## Policies > ### Security` section
