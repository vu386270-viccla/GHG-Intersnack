...
Invoke `the rune-ba skill` with the user's project description.
...
Invoke `the rune-research skill` to find:
...
Invoke `the rune-plan skill` with the Requirements Document from Phase 1 and research from Phase 2.
...
- Invoke `the rune-design skill` to generate design system
...
**Parallelization**: If plan has 3+ independent modules → invoke `the rune-team skill` to implement in parallel using worktrees.
...
Invoke `the rune-test skill` to generate tests based on acceptance criteria from Phase 1:
...
Invoke `the rune-verification skill` to run ALL checks:
...
5. **Security**: `the rune-sentinel skill` quick scan — no critical issues

<HARD-GATE>
If ANY check fails → fix the issue (invoke the rune-fix skill) and re-verify.
