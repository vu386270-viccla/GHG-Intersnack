...
```
REQUIRED SUB-SKILL: the rune-verification skill
...
Block deploy if ANY of:
  ...
  [ ] Build fails
  [ ] sentinel CRITICAL issues detected (invoke the rune-sentinel skill if not already run)
...
```
REQUIRED SUB-SKILL: the rune-browser-pilot skill
...
```
REQUIRED SUB-SKILL: the rune-watchdog skill
...
```
REQUIRED SUB-SKILL: the rune-marketing skill
...
If user requested video content:
  REQUIRED SUB-SKILL: the rune-video-creator skill
...
|------|----------|------------|
| Test Gate | verification output showing all green | Run the rune-verification skill first |
| Security Gate | sentinel output with no CRITICAL findings | Run the rune-sentinel skill first |
...
| Marketing assets (copy, social, SEO) | Markdown (inline) | Generated by `the rune-marketing skill`, presented in Phase 5 |
