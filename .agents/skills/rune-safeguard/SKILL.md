...
Call `the rune-scout skill` targeting the specific module. Ask scout to return:
...
Call `the rune-verification skill` and explicitly pass the characterization test file path.
...
### Next Step
Safe to proceed with: `the rune-surgeon skill` targeting [module]
```
