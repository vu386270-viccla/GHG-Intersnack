...
<HARD-GATE>
- Tests MUST pass (via `the rune-verification skill`) before deploy runs
...
Call `the rune-verification skill` to run the full test suite and build.

```
If verification fails → STOP. Do NOT proceed. Report failure with test output.
```

Call `the rune-sentinel skill` to run security scan.
...
If `the rune-browser-pilot skill` is available, call it to take a screenshot of the deployed URL for visual confirmation.

### Step 5 — Monitor

Call `the rune-watchdog skill` to set up post-deploy monitoring alerts on the deployed URL.
