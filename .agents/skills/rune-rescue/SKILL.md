...
```
REQUIRED SUB-SKILL: the rune-autopsy skill
...
Check: does CLAUDE.md exist in project root?
  If NO:
    REQUIRED SUB-SKILL: the rune-onboard skill
...
```
REQUIRED SUB-SKILL: the rune-dependency-doctor skill
...
```
REQUIRED SUB-SKILL: the rune-journal skill
...
REQUIRED SUB-SKILL: the rune-session-bridge skill
...
```
REQUIRED SUB-SKILL: the rune-safeguard skill
...
```
REQUIRED SUB-SKILL: the rune-surgeon skill
→ Invoke `surgeon` with:
...
```
REQUIRED SUB-SKILL: the rune-review skill
...
|------|----------|------------|
| Autopsy Gate | autopsy report with health score before planning | Run the rune-autopsy skill first |
| Safety Gate | safeguard characterization tests passing before surgery | Run the rune-safeguard skill first |
...
| Characterization tests | Source files | Written by `the rune-safeguard skill` per module |
