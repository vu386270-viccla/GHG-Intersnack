...
Call `the rune-scout skill` to scan the codebase. Ask scout to extract:
...
Call `the rune-trend-scout skill` with the product category to identify:
- Top 3 competitors and their positioning
- Current market trends relevant to this product
- Differentiators to emphasize

Call `the rune-research skill` for:
...
Call `the rune-asset-creator skill` to generate:
- OG image (1200x630px) — product name, tagline, brand colors
- Twitter card image (1200x628px)
- Product Hunt thumbnail (240x240px)

Call `the rune-video-creator skill` to produce:
- 60-second demo video script (screen recording plan)
- Shot list with timestamps

Call `the rune-slides skill` to generate presentation decks for launch demos, sprint reviews, or investor pitches.

If `the rune-browser-pilot skill` is available, capture screenshots of the running app to use as real product imagery.
