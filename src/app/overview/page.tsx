...
import SkeletonOverview from '@/components/layout/SkeletonOverview';
...
function MiniDonut({ segments, size = 120, thickness = 22, centerLabel, centerSub }: {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}
      ...
      {centerLabel && <>
        <text x={cx} y={cy - 4} textAnchor="middle" fontSize="15" fontWeight="800" fill="var(--ov-text)" fontFamily="'Caveat',cursive">{centerLabel}</text>
        {centerSub && <text x={cx} y={cy + 11} textAnchor="middle" fontSize="8" fill="var(--ov-text-dim)" fontWeight="600">{centerSub}</text>}
      </>}
    </svg>
  );
}
