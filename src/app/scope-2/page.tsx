          ...
          <div className="card" style={{ padding: '12px 16px', marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--color-text-secondary)' }}>
                Scope 2 yearly overview - select a year, then review monthly factory detail
              </div>
            ...
            {annualRows.length > 0 ? (
              <BarChart
                data={annualRows.map(r => ({
                  label: String(r.year),
                  values: [{ key: 's2', value: r.s2, color: r.year === selectedYear ? scopeColor : '#93C5FD' }],
                }))}
                legendLabels={{ s2: 'Scope 2 (tCO2e)' }}
                height={210}
                showValueLabels
              />
            ) : (
