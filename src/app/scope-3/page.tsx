...
import { DEMO_SCOPE_SUMMARIES, DEMO_FACTORY_SUMMARIES, formatTCO2e } from '@/lib/demo-data';
...
export default function Scope3Page() {
  const scopeData = DEMO_SCOPE_SUMMARIES.find(s => s.scope === 'scope_3')!;
    ...
    <div>
      ...
      <div className="page-header">
        <div className="page-title">
          ...
          Scope 3
          <span className="page-title-accent">Chuỗi giá trị</span>
        </div>
        <div className="page-subtitle">
          Phát thải gián tiếp khác trong chuỗi giá trị — SBTi cam kết theo dõi 3 Categories
        </div>
      </div>
      ...
      <div className="grid-4 stagger-children mb-xl">
        ...
        <div className="card kpi-card animate-fade-in-up">
          ...
          <div className="kpi-card-label">SBTi Categories</div>
        </div>
      </div>
      ...
      <div className="grid-3 stagger-children mb-xl">
        {scopeData.categories.map((cat, i) => {
          ...
          return (
            <div key={cat.category} className="card animate-fade-in-up" style={{
              borderLeft: `4px solid ${catColors[i]}`,
            }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-md)' }}>
                {cat.label}
              </div>
      ...
      <div className="grid-1-2 mb-xl">
        <div className="card animate-fade-in-up">
          <div className="card-header">
            <div className="card-title">Phân bổ Scope 3</div>
          </div>
          <DonutChart
        ...
        {/* Factory Scope 3 Comparison */}
        <div className="card animate-fade-in-up">
          <div className="card-header">
            <div className="card-title">So sánh nhà máy — Scope 3</div>
          </div>
          <BarChart
            data={DEMO_FACTORY_SUMMARIES.map(fs => ({
              label: `${fs.factory.code} ${fs.factory.country === 'India' ? '🇮🇳' : '🇻🇳'}`,
              values: [
                { key: 'scope_3', value: fs.scope3, color: scopeColor },
              ],
            }))}
      ...
      <div className="card animate-fade-in-up">
        <div className="card-header">
          <div className="card-title">Xu hướng Scope 3 hàng tháng</div>
        </div>
        <TrendLine
          data={DEMO_FACTORY_SUMMARIES[0].monthlyTrend.map((_, i) => ({
            ...
            values: [
              {
                key: 'total_s3',
                value: DEMO_FACTORY_SUMMARIES.reduce((sum, fs) => sum + fs.monthlyTrend[i].scope3, 0),
                color: scopeColor,
              },
            ],
          }))}
      ...
      {/* SBTi Note */}
      <div className="card mt-xl animate-fade-in-up" style={{
        background: 'linear-gradient(135deg, #8CB92D11, #8CB92D05)',
        border: '1px solid #8CB92D33',
      }}>
        <div style={{ display: 'flex', gap: 'var(--space-lg)', alignItems: 'flex-start' }}>
          <div style={{ fontSize: '32px' }}>🎯</div>
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: '22px', color: scopeColor, fontWeight: 700, marginBottom: '4px' }}>
              SBTi Commitment
            </div>
